...
@elseif($payment == 'Bitcoin')
    ...
    <div class="row">&nbsp;</div>
    <div style="text-align: initial;">
        <label><strong>Price: </strong> $ 1 equibale a {{number_format(floatval($transaction->price_cryptocurrency), 2, '.', ',')}} {{$transaction->baseAsset}}</label> <br>
        <label><strong>Total a Pagar: </strong> {{number_format((1 / floatval($transaction->price_cryptocurrency)), 8, '.', ',') }} {{$transaction->baseAsset}}</label> <br>
